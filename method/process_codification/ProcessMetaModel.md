...
**Б. В графе Design** задается иерархия (дерево иерархии) и фиксируется наличие схемы процесса. Введем два типа схем (для начала): структурные (Str = structural diagram) и «добавленного качества» (VAD): 
...
**Г. На начальном этапе id процесса** будем вычислять по названию процесса путем замены пробелов на “_”, например, «Разработка схемы» даст id = Разработка_схемы. 
...
Однако, нам важно, чтобы id был человеко - читаемым. Для упрощения предлагаю для номерных процессов оставлять только номер процесса, а для ненамеренных имя процесса с заменой пробелов на «_».
