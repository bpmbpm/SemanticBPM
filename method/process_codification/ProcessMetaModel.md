...
2.2. Набор графов – схем (diagram) для каждого процесса, графы «Dia» (для каждого процесса свой граф, его имя начинается с "g"). 
### Б. Граф Design
...
В коде (id) **процесса указываем первую букву «p»**, в коде (id) **графа первую букву «g»**. 
...
Четверки (квадры) можно кодировать в формате N-Quads или TriG . Квады vs [Именованные графы](https://en.wikipedia.org/wiki/Named_graph): 
- Квады имеют формат: субъект предикат объект контекст; 
- Именованные графы: субъект предикат объект graphname. 
Не совсем понял отличие, но GPT говорит, что есть [Quads.md](https://github.com/bpmbpm/SemanticBPM/blob/main/method/process_codification/GPT_quads_vs_named_graph.md), однако в нашем случае их можно считать синонимами. Причем сама [wiki пишет](https://en.wikipedia.org/wiki/Named_graph):
...
### Алгоритм формирования деревьев (графов)
