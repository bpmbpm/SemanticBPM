...
Фактически выше мы обсуждали как с помощью конкретного механизма кодирования (id/iri/url) и представдения знаний в атомарных записях (Triples vs Quads), что есть  
MDM (meta-data management), реализовать механизм "другого" MDM (master data management). Децимальные номера изделий (ЕСКД \ ЕСПД), схема деления изделия на составные части (ГОСТ 2.711–82) это ровно про оба этих MDM.
