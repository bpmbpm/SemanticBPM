## Repo MetaModel (TriG)
**МетаМодель Репозитария схем процессов на TriG (quads)**  
Считаем синонимами: схема \ диаграмма \ граф. МетаМодель Репозитария, включая МетаМодель Схемы Процесса (DiaMetaModel).  
...
2.2. Набор графов – схем (diagram) для каждого процесса, графы «Dia» (для каждого процесса свой граф, его имя начинается с "gp"). 
### Б. Граф Design
...
В коде (id) **процесса указываем первую букву «p»**, в коде (id) **графа первую букву «gp»**: граф процесса, а потом будет еще графы (схемы) других сущеностей: орг-структура, схема инструментов (ИТ-систем), документов и т.п.  
Чтобы показать иерархию процессов используем отношение vad:hasParent (обратное vad:hasChild). В дереве процессов (графе Design) всегда есть ровно один родитель, т.е. нет повторов и соблюдается уникальность кода, id. Причем он (id материнского процесса) может быть вычислен: если номер процесса "1_3", то его родительский процесс "1" (отбразываем последнюю цифру до разделителя). Однако это верно только для номерных процессов.  
Процесс номерной – если он имеет детализацию. Для процессов верхнего уровня (1, 2, 3 и т.д.) родительский элемент p0 (ProcessRoot).  
В дереве процессов будет фактически две части: верхняя - дерево номерных процессов (у которых есть детализация) и нижняя - дерево ненумерных процессов (без детализации).
...
Соответственно "p1_2_Process_1_2" и "p1_2_Process_1_2#2" одного графа (схемы) процесса отображаются (сопоставляются) с одним и тем же p1_2_Process_1_2 в графе дерева процессов (граф Design).  
Нужно как-то учитывать в запросах использование «#», например, при запросах: 
...
- кодировать элемент в соответствии с его типом, получая составной элемент, например, StrType.22 или VadType.33. В visio к имени .master (мастер-шейпа) добавляется следующий порядковый номер;
- абсолютная нумерация типа fun(x,y,z), где x-номер схемы (код схемы процесса), y-код элемента схемы, полученный из имени подпроцесса, например, заменой пробелов на '_ ' (или иной способ кодирования элемента), z - номер одноименного элемента на схеме, т.е. аналог #2.  
...
### Д. Квадры (созвучнее, чем квады)
Использование четверок вместо троек привязывает триплет к конкретной схеме процесса. Смотрим на схему процесса VAD и все что на ней нарисовано будет закодировано триплетами под идентификатором этого графа (id схемы процесса). 
...
### Е. Алгоритм формирования деревьев (графов)
Алгоритм формирования дерева схем (и поддерева объектов схемы), дерева иерархии процессов. Рассмотрим пост-фактум парсинг графического файла со схемой процесса. На входе файл «.drawio» или yEd и т.п., а на выходе TriG (RDF Quads).  
Принцип: «один графический файл схемы = один RDF \ TriG файл» сохраняется. Это удобно, т.к. при изменении схемы процесса мы вносим изменение только в один файл TriG. В одноименный TriG складываем результат разбора схемы процесса (парсер «.drawio» или yEd). Сколько файлов схем, - столько и файлов TriG.  
На основе каждого TriG каждой схемы процесса (графы «Dia») делаем корректировки в Граф Design. Это граф, который фактически объединяет все графы «Dia» с исключением дублей (см. п. В. Дубли процессов, объектов). В целом Граф Design (дерево процессов и их объектов) – это и есть treeview в правом верхнем боковике основного экрана Semantic BPM (Semantic ARIS).  
Рисунок отражает три компонента: дерево процессов (TriG-граф иерархии процессов и их элементов - без повторов \ дублей), набор TriG-графов схем процессов (одна графическая схема = один TriG-граф), набор графических файлов процессов (drawio, yEd, visio, svg и т.п.):
<img src="RepoModel.png" width="1000" />
### Итого
Получились в итоге следующие составляющие Repo MetaModel: 
- две области схем \ графов: дерево процессов (все процессы в иерархии, т.е. без дублей) и дерево схем процессов;
- DiaMetaModel, включающая формат схемы процесса: заголовок процесса (желтый прямоугольник вверху схемы) и составные части процесса (элементы, VAD-кораблики), которые при наличие дублей их кодируют через символ "#".
